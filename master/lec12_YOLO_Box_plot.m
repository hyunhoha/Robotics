...
while ishandle(h) % if you shutdown figure window, then this code shutdown too
    ...
    for ii=1:sz(1)
        ...
        scalex=inputSize(2)/nx; scaley=inputSize(1)/ny;
        
        xs = fix(boxes(ii,1)/scalex);
        xe = fix(xs+boxes(ii,3)/saclex)-2;
        ys = fix(boxes(ii,2)/scaley);
        ye = fix(ys+boxes(ii,4)/scaley);
        
        dep_mm = double(depth_value(ii))*1000*ones(4,4);
        pX=[xs, xs, xe, xe];
        pY=[ye, ys, ys, ye];
        pX=pX-K(1,3)+0.5;
        pY=pY-K(2,3)+0.5;
